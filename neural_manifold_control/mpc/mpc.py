...
p.add_argument('--sqr_matrix_diags',nargs='+',default=[10e1,10e1,1e-1],type=float)
p.add_argument('--n_horizon',type=int,default=30)
args = p.parse_args()
...
for time_step in tqdm(range(n_steps)):
    ...
    v_optimal = K*(Z0-ref_traj[time_step,:])
    #v_optimal = V_train[time_step,:]
    # Save for inspection
    ...
    if time_step == 0:
        ...
        full_state = 0.05*torch.from_numpy(spike_collector[time_step]).to(torch.float32)+(1-0.05)*full_state
    # Encode spikes into neural manifold space
    z_np1, _ ,_ = ldm.encode_x(full_state.reshape(1, -1))
    # lowpass filter
    if time_step==0:
        Z_lowpass[time_step] = z_np1.detach().numpy()
    else:
        Z_lowpass[time_step] = Z_lowpass[time_step-1]+0.1*(z_np1.detach().numpy()-Z_lowpass[time_step-1])
    Z[time_step] = z_np1.detach().numpy()

    # Update latent state for MPC optimization
    #Z0 = Z_lowpass[time_step]
    Z0 = Z[time_step]
# Save data
'''
...
ax[0].plot(Z_lowpass[:,0],color='darkcyan',alpha=0.6)
ax[1].plot(ref_traj[:,1],color='black',alpha=0.5)
ax[1].plot(Z[:,1],color='red',alpha=0.5)
ax[1].plot(Z_lowpass[:,1],color='darkcyan',alpha=0.6)
