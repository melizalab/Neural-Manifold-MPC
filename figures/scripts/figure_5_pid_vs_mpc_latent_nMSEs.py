...
# Init V storage
mpc_RMS_v1 = np.zeros_like(mpc_nMSE_z1)
mpc_RMS_v2 = np.zeros_like(mpc_nMSE_z1)
p_RMS_v1 = np.zeros_like(mpc_nMSE_z1)
p_RMS_v2 = np.zeros_like(mpc_nMSE_z1)
...
for i,prob in enumerate(probs):
    for sample in range(n_samples):
        for trial in range(n_trials):
            ...
            # Get V Input
            p_RMS_v1[i,sample,trial] = RMS(p_data['V'][:,0])
            p_RMS_v2[i,sample,trial] = RMS(p_data['V'][:,1])

            mpc_RMS_v1[i,sample,trial] = RMS(mpc_data['V'][:,0])
            mpc_RMS_v2[i,sample,trial] = RMS(mpc_data['V'][:,1])
...
p_RMS = (p_RMS_v1+p_RMS_v2)/2
mpc_RMS = (mpc_RMS_v1+mpc_RMS_v2)/2

for i, prob in enumerate(probs):
    print(f'Best PID sample for prob {prob}: {p_nMSE[i].mean(1).argmin()}')
'''
Get best trials
prob = 0.01
...
prob = 0.05
p_nMSE[1].mean(1).argmin() = 0

prob = 0.1
p_nMSE[2].mean(1).argmin() = 5

prob = 0.2
p_nMSE[3].mean(1).argmin() = 6

prob = 0.3
p_nMSE[4].mean(1).argmin() = 8

prob = 0.4
p_nMSE[5].mean(1).argmin() = 8

prob = 0.5
p_nMSE[6].mean(1).argmin() = 5

prob = 0.6
p_nMSE[7].mean(1).argmin() = 0
'''
...
for i,prob in enumerate(probs):
    ...
    data = [p_RMS[i].mean(1),mpc_RMS[i].mean(1)]
...
plt.tight_layout()
plt.savefig('figures/pid_figures/figure_5_B_pid_vs_mpc_latent_RMSs.pdf')
breakpoint()
...
for i,prob in enumerate(probs):
    positions = np.array([-0.20,0.20])+i*2
    data = [p_nMSE[i].mean(1),mpc_nMSE[i].mean(1)]
...
#plt.grid(True, linestyle="--", alpha=0.5)
plt.tight_layout()
plt.savefig('figures/pid_figures/figure_5_B_pid_vs_mpc_latent_nMSEs.pdf')